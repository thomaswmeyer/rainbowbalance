...
The wizard's three spells are told apart the way the streak tells them apart:
`casts` carries `_k`, `sparks.js` draws frost blue, gold and red off it, and
`cast()` takes the same number. A sound that lumped them together would
undo work the picture had already done.
Levels below are peak amplitude, measured out of a browser with the thing at
the front of the field.

| | what it is | peak |
|---|---|---|
| a recruit out of a gate | a triangle rising a fifth, and the puff of the gate. C4 for rainicorns, F4 for sunicorns | 0.16 |
| horn on horn | a wide band of noise falling, with a short ring over it | 0.19 |
| a unicorn falls | a sawtooth with the pitch falling out of it, and the body landing | 0.32 |
| a wizard's frost | two sines sweeping up out of each other, and the crystals | 0.17 |
| a wizard's bolt | a sawtooth falling out of the top with a wide burst behind it — it does damage, so it lands rather than settles | 0.24 |
| a wizard's rage | a sawtooth swelling upward under one of the caster's own: given, not done to | 0.19 |
| a side buys a power | a low sweep up an octave and two sustained notes over it | 0.13 |
| the smiting hand | a clap, a sawtooth falling to 42 Hz, and a sine sub under it | 0.81 |
| the freezing hand | three chimes an instant apart over the hiss of it going hard | 0.34 |
| a promotion | the D triad of the current key, rising, three notes | 0.14 |
| a castle taken | a bell, the same triad an octave up under it, and a shimmer over the walls | 0.60 |
| a claim broken | a hollow square knock falling, with the stone going out of it | 0.26 |
| a side wins | the triad and the octave, long, over the drone being taken away | 0.61 |
...
A power bought is the other way round from everything else here: it has no
position. It happened to a side, not at a place, and the shower goes up over
every castle that side holds. So it is the one sound placed by side alone, and
it goes to the music bus rather than the field's, because what it says is that
the run has just got harder and that is the score's job to say.
