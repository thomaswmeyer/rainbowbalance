...
void main(){
  ...
  // exactly one place: every call site inlines its own copy, and three
  // copies made the whole program slow for every pixel, not only the ones
  // that took the extra branch. So the sky this ray sees, or would see past
  // what it hits, is computed once here and used by every case. It is only
  // paid for where the fog would show it.
  float dist = onCastle ? tc : t;
  float fog = dist < 0.0 ? 1.0 : clamp(dist * dist * FOG, 0.0, 1.0);
  ...
  if (BOW_ON == 1) {
    ...
    float sky = smoothstep(0.0, 0.006, p.y - FOOT) * (onCastle ? 0.0 : 1.0);
